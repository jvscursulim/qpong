...
def draw_lose_scene(screen):
    ...
    text = font.gameover_font.render("Game Over", 1, globals.WHITE)
    text_pos = text.get_rect(center=(globals.WINDOW_WIDTH/2, globals.WIDTH_UNIT*10))
    screen.blit(text, text_pos)

    text = font.replay_font.render("Classical computer", 5, globals.WHITE)
    text_pos = text.get_rect(center=(globals.WINDOW_WIDTH/2, globals.WIDTH_UNIT*22))
    screen.blit(text, text_pos)

    text = font.replay_font.render("still rules the world", 5, globals.WHITE)
...
def draw_win_scene(screen):
    ...
    text = font.gameover_font.render("Congratulations!", 1, globals.WHITE)
    text_pos = text.get_rect(center=(globals.WINDOW_WIDTH/2, globals.WIDTH_UNIT*10))
    screen.blit(text, text_pos)

    text = font.replay_font.render("You demonstrated quantum advantage", 5, globals.WHITE)
    text_pos = text.get_rect(center=(globals.WINDOW_WIDTH/2, globals.WIDTH_UNIT*22))
    screen.blit(text, text_pos)

    text = font.replay_font.render("for the first time in human history!", 5, globals.WHITE)
    text_pos = text.get_rect(center=(globals.WINDOW_WIDTH/2, globals.WIDTH_UNIT*27))
    screen.blit(text, text_pos)
